...
var MFP = {
	...
	addModule: function( name, data, row ){		
		var self	= this,
			...
			$tabs	= jQuery('<ul id="c-tabs-' + self._row + '" class="nav nav-tabs">')
				...
				.append('<li><a data-toggle="tab" href="#tab-' + self._row + '-attribs"><i class="glyphicon glyphicon-list"></i> <span><?php echo $tab_attributes; ?></span></a></li>')
				.append('<li><a data-toggle="tab" href="#tab-' + self._row + '-options"><i class="glyphicon glyphicon-list"></i> <span><?php echo $tab_options; ?></span></a></li>')
				.append(self._hasFilters?'<li><a data-toggle="tab" href="#tab-' + self._row + '-filters"><i class="glyphicon glyphicon-filter"></i> <span><?php echo $tab_filters; ?></span></a></li>':'')
				...
				.append('<li><a data-toggle="tab" href="#tab-' + self._row + '-configuration"><i class="glyphicon glyphicon-wrench"></i> <span><?php echo $tab_configuration; ?></span></a></li>'),
			$tc		= jQuery('<div class="tab-content">')
				...
				.append(jQuery('<div id="tab-' + self._row + '-settings" class="tab-pane active">')
					.append(jQuery('<table class="table table-tbody">')
						...
						.append(jQuery('<tr>')
							.append('<td><?php echo $entry_display_always_as_widget; ?></td>')
							.append('<td data-name="display_always_as_widget"></td>')
						)
						.append(jQuery('<tr>')
	...
	_initModule: function( $module, data ) {
		...
		$module.find('[data-name="display_always_as_widget"]')
			.append( self.createField( 'select', '[display_always_as_widget]', data['display_always_as_widget'], {
				'multiOptions' : {
				'items' : {
					'0' : '<?php echo $text_no; ?>',
					'1' : '<?php echo $text_yes; ?>'
		...
		/**
		 * Zmień pozycję jeśli wybrano wyświetlanie jako widget
		 */
		$module.find('[data-name="display_always_as_widget"] select').change(function(){
			var $position = $module.find('[data-name="position"] select');
			
			if( jQuery(this).val() == '1' ) {
				$position.find('option').removeAttr('selected').each(function(i){
					if( jQuery(this).attr('value') == 'content_top' ) {
						$position.prop('selectedIndex', i).addClass('hide').trigger('change');
						$position.next().removeClass('hide');
						
						return false;
					}
				});
			} else {
				$position.removeClass('hide').prop('selectedIndex',0).trigger('change');
				$position.next().addClass('hide');
			}
		});
		
		if( $module.find('[data-name="display_always_as_widget"] select').val() == '1' ) {
			$module.find('[data-name="display_always_as_widget"] select').trigger('change');
		}
		
		$module.find('select[name="mega_filter_module[position]"]').change(function(){
			$module.find('[data-name="inline-horizontal"]').parent()[jQuery(this).val()=='content_top'&&$module.find('[data-name="display_always_as_widget"] select').val() != '1'?'show':'hide']();
		}).trigger('change');
	...
	startExternalViewLoad: function(){
		...
		jQuery('#mf-save-form').attr('disabled', true);
	},
	...
	stopExternalViewLoad: function(){
		...
		if( this._externalViews <= 0 ) {
			...
			jQuery('#mf-save-form').removeAttr('disabled');
		}
	},
	...
	createContainer: function( id, type ) {
		...
		return jQuery('<div id="tab-' + self._row + '-' + id + '" class="tab-pane">')
			.append(type=='base_attribs'||type=='configuration'?'':jQuery('<div class="pull-left input-group mf-filter" style="padding:5px">')
